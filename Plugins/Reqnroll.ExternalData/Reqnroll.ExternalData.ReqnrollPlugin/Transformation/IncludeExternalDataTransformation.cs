...
namespace Reqnroll.ExternalData.ReqnrollPlugin.Transformation
{
    public class IncludeExternalDataTransformation : ScenarioTransformation
    {
        ...
        private IEnumerable<Tag> GetSourceFeatureTags() => 
            _sourceDocument.ReqnrollFeature.Tags ?? new Tag[0];
        ...
        private Scenario GetTransformedScenario(Scenario scenario, ExternalDataSpecification specification, string[] examplesHeaderNames = null, string examplesKeyword = "Examples")
        {
            ...
            var exampleRows = exampleRecords.Items
                                            .Select(rec => new Gherkin.Ast.TableRow(default, exampleRecords.Header.Select(h => new TableCell(default, rec.Fields[h].AsString(_featureCultureInfo))).ToArray()))
                                            .ToArray();
        ...
        private Examples CreateExamplesBlock(string[] headerNames, Gherkin.Ast.TableRow[] exampleRows, string keyword)
        {
            keyword ??= "External Examples";
            var name = "External Examples";
            var tableHeader = new Gherkin.Ast.TableRow(default, headerNames.Select(h => new TableCell(default, h)).ToArray());
            return new Examples(new Tag[0], default, keyword, name, "", tableHeader, exampleRows);
        }
    }
}
