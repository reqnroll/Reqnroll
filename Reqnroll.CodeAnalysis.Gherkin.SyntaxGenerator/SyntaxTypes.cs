namespace Reqnroll.CodeAnalysis.Gherkin.SyntaxGenerator;

internal static class SyntaxTypes
{
    public const string SyntaxNode = "Reqnroll.CodeAnalysis.Gherkin.Syntax.SyntaxNode";
    public const string SyntaxToken = "Reqnroll.CodeAnalysis.Gherkin.Syntax.SyntaxToken";
    public const string SyntaxTokenList = "Reqnroll.CodeAnalysis.Gherkin.Syntax.SyntaxTokenList";
    public const string StructuredTriviaSyntax = "Reqnroll.CodeAnalysis.Gherkin.Syntax.StructuredTriviaSyntax";
    public const string SyntaxSlotAttribute = "Reqnroll.CodeAnalysis.Gherkin.Syntax.SyntaxSlotAttribute";
    public const string SyntaxNodeAttribute = "Reqnroll.CodeAnalysis.Gherkin.Syntax.SyntaxNodeAttribute";
}
