...
{
    public abstract class MsTestGeneratorProviderTestsBase
    {
        ...
        protected abstract MsTestGeneratorProvider CreateProvider(CodeDomHelper codeDomHelper);
        protected abstract string ExpectedDoNotParallelizeAttributeName { get; }
        protected abstract string ExpectedPriorityAttributeName { get; }

        [Fact]
        public void MsTestGeneratorProvider_WithFeatureWithoutDeploymentItem_GeneratedClassShouldNotIncludeDeploymentItemForPlugin()
        {
            // ARRANGE
            ...
            var sampleTestGeneratorProvider = CreateProvider(new CodeDomHelper(CodeDomProviderLanguage.CSharp));
        ...
        [Fact]
        public void MsTestGeneratorProvider_WithFeatureContainingDeploymentItem_GeneratedClassShouldIncludeDeploymentItemForPlugin()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
        ...
        [Fact]
        public void MsTestGeneratorProvider_ShouldHaveParallelExecutionTrait()
        {
            var sut = CreateProvider(new CodeDomHelper(CodeDomProviderLanguage.CSharp));
        ...
        [Fact]
        public void MsTestGeneratorProvider_WithFeatureWithMatchingTag_ShouldAddNonParallelizableAttribute()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            @nonparallelizable
            ...
            var provider = CreateProvider(new CodeDomHelper(CodeDomProviderLanguage.CSharp));
            ...
            attributes.Should()
                      .ContainSingle(a => a.Name == ExpectedDoNotParallelizeAttributeName)
                      .Which.Arguments.Count.Should()
                      .Be(0);
        }

        [Fact]
        public void MsTestGeneratorProvider_WithFeatureWithNoMatchingTag_ShouldNotAddNonParallelizableAttribute()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            Feature: Sample feature file
            ...
            // ASSERT
            var attributes = code.Class().CustomAttributes().ToArray();
            attributes.Should().NotContain(a => a.Name == ExpectedDoNotParallelizeAttributeName);
        }

        [Theory]
        [InlineData(true)]
        [InlineData(false)]
        public void MsTestGeneratorProvider_WithPriorityTag_ShouldAddPriorityAttribute(bool testFeatureTag)
        {
            // ARRANGE
            ...
            testMethod.CustomAttributes().Should().ContainSingle(a => a.Name == ExpectedPriorityAttributeName && (int)a.ArgumentValues().First() == 1);
        }

        [Fact]
        public void MsTestGeneratorProvider_WithoutPriorityTag_ShouldntAddPriorityAttribute()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            Feature: Sample feature file
            ...
            // ASSERT
            var testMethod = code.Class().Members().Single(m => m.Name == "SimpleScenario");
            testMethod.CustomAttributes().Should().NotContain(a => a.Name == ExpectedPriorityAttributeName);
        }

        [Fact]
        public void MsTestGeneratorProvider_WithScenarioOutline_ShouldGenerateASingleParametrizedTest()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            Feature: Sample feature file
        ...
        [InlineData(true)]
        [InlineData(false)]
        public void MsTestGeneratorProvider_WithScenarioOutline_ShouldGenerateDisplayNamePropertyOfEachDataRowPerConfiguration(bool disableFriendlyName)
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            Feature: Sample feature file
        ...
        [Fact]
        public void MsTestGeneratorProvider_WithScenarioOutline_ShouldSkipIgnoredExamples()
        {
            // ARRANGE
            var document = ParseDocumentFromString(@"
            Feature: Sample feature file
        ...
        [Fact]
        public void MsTestGeneratorProvider_ScenarioTagMatching_ShouldAddDoNotParallelizeAttributeToMethodOnly()
        {
            var document = ParseDocumentFromString(
                @"
            Feature: Sample feature file
            ...
            var code = featureGenerator.GenerateUnitTestFixture(document, "TestClassName", "Target.Namespace").CodeNamespace;
            code.Class().CustomAttributes().Should().NotContain(a => a.Name == ExpectedDoNotParallelizeAttributeName);

            var method = code.Class().Members().Single(m => m.Name == "IsolatedScenario");
            method.CustomAttributes().Should().Contain(a => a.Name == ExpectedDoNotParallelizeAttributeName);
        }

        protected ReqnrollDocument ParseDocumentFromString(string documentSource, CultureInfo parserCultureInfo = null)
        {
            ...
            {
                var document = parser.Parse(reader, new ReqnrollDocumentLocation($"dummy_Location_for_{GetType().Name}"));
