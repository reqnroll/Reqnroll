...
#pragma warning restore CS0618
    {
        ...
        public BindingInvoker(ReqnrollConfiguration reqnrollConfiguration, IErrorProvider errorProvider, IBindingDelegateInvoker bindingDelegateInvoker)
        {
        ...
        public virtual async Task<object> InvokeBindingAsync(IBinding binding, IContextManager contextManager, object[] arguments, ITestTracer testTracer, DurationHolder durationHolder)
        {
            ...
            try
            {
                ...
                if (reqnrollConfiguration.TraceTimings && stopwatch.Elapsed >= reqnrollConfiguration.MinTracedDuration)
                {
        ...
        protected void EnsureReflectionInfo(IBinding binding, out MethodInfo methodInfo, out Delegate bindingAction)
        {
            ...
            throw new ReqnrollException("The binding method cannot be used for reflection: " + binding);
        }

        protected virtual Delegate CreateMethodDelegate(MethodInfo method)
        {
            if (AsyncMethodHelper.IsAsyncVoid(method)) 
                throw new ReqnrollException($"Invalid binding method '{method.DeclaringType!.FullName}.{method.Name}()': async void methods are not supported. Please use 'async Task'.");
