...
public class TestCaseExecutionTracker
{
    ...
    public async Task ProcessEvent(ScenarioFinishedEvent scenarioFinishedEvent)
    {
        ...
        if (AttemptId == 0)
        {
            ...
            await _publisher.PublishAsync(Envelope.Create(testCase)); // using the BufferedMessagePublisher will ensure that this is published before any other messages related to this TestCase (such as TestCaseStarted, etc)
        }
